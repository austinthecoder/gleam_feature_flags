...
pub fn main() {
  ...
  // why do i block here ?
  let _pid = process.start(running: thing, linked: True)
...
pub fn loop(
  my_subject: Subject(ChannelRequest),
  parent_subject: Subject(ChannelResponse),
  channels: List(Int),
) {
  ...
  let new_channels = case msg {
    Allocate -> {
      ...
      let assert [next_available, ..rest] = channels
